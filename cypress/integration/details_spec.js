describe('Details page user flow', () => {
    ...
    it('should display all 40 movie posters upon successful network request', () => {
        cy.fixture("single_movie_data.json").as("singleMovieData");
        cy.intercept('GET', 'https://rancid-tomatillos.herokuapp.com/api/v2/movies/726739', "singleMovieData");
    });

    it('should display an error message when the network request is unsuccessful', () => {
        cy.fixture("error_data.json").as("errorData");
        cy.intercept('GET', 'https://rancid-tomatillos.herokuapp.com/api/v2/movies/726739',"errorData");
        cy.get('#726739').click()
        .get('.error-message')
